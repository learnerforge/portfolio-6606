      ...
      >
        ...
        <div className="relative p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="rounded-md bg-black px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-white">
              Flagship
            </span>
            <span className="rounded-md bg-white px-2.5 py-1 font-mono text-[11px] font-semibold text-[#1a1a2e]">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {featured.highlight.map((h) => (
              <span key={h} className="rounded-lg bg-black px-3 py-1.5 text-xs font-semibold text-white">
                {h}
              </span>
            ))}
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <a href={featured.github} target="_blank" rel="noopener noreferrer" className="btn btn-primary px-5 py-2.5 text-sm text-white">
              Source on GitHub <Icon name="arrow" size={13} />
            </a>
            <button onClick={() => os.open('ai-lab')} className="btn bg-white px-5 py-2.5 text-sm font-bold text-[#1a1a2e] transition-colors hover:bg-neutral-100">
              Open in AI Lab
            </button>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {others.map((p, i) => (
          <motion.a
            ...
            className="group flex flex-col rounded-xl border border-line bg-bg p-4 transition-colors hover:border-accent-3/40"
          >
